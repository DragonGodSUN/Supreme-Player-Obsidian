...
const Shop = {
  showShop(plugin) {
    ...
    modal.titleEl.setText(plugin.dataStore.shopConfig?.shopName || '🎴 道具商城');
    ...
    infoDiv.innerHTML = '🎴 稀有道具卡：' + stats.rareItemCards + ' &nbsp;&nbsp; 🌠 传奇道具卡：' + stats.legendaryItemCards;
    ...
    for (const item of items) {
      const canAfford = (item.rarity === 'rare' && stats.rareItemCards >= item.price) ||
      ...
      const categoryTag = item.category === 'system' ?
        '<span style="background: #00aaff20; color: #00aaff; padding: 2px 6px; border-radius: 3px; font-size: 10px; margin-left: 5px;">系统</span>' :
        '<span style="background: #ffaa0020; color: #ffaa00; padding: 2px 6px; border-radius: 3px; font-size: 10px; margin-left: 5px;">外部</span>';

      itemDiv.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <div><span style="font-size: 20px; margin-right: 10px;">${item.icon}</span><strong>${item.name}</strong>${categoryTag}${item.rarity === 'legendary' ? '<span style="color: #ffd700; margin-left: 5px;">🌠</span>' : '<span style="color: #9966ff; margin-left: 5px;">🎴</span>'}</div>
  ...
  showPurchaseConfirm(plugin, item, shopModal) {
    ...
    content.innerHTML = `
      ...
      <div style="background: var(--background-secondary); padding: 10px; border-radius: 5px; margin-bottom: 15px;">
        确定要购买此商品吗？<br>将消耗 <strong>${item.rarity === 'legendary' ? '🌠' : '🎴'} ${item.price}</strong> 张${item.rarity === 'legendary' ? '传奇' : '稀有'}道具卡
      </div>
    `;
